const portfolioData = [{
    id: 1,
    img: require("../images/soundBeamprj.png"),
    Heading: "Sound Beam Project",
    brief: "Website for a technology company that creates technology that bring the sensation of sound.",
    Description: "Project for a technology company that creates technology that bring the sensation of sound. This company is mainly focused on making premium quality headphones.\nThe website boasts the premium quality of their products with the help of complementing colors and fonts that just match the vibes and have suttle animation for making the user feel involved",
  },
  {
    id: 2,
    img: require("../images/binanceprj.png"),
    addClass: "RL",
    Heading: "Binance Project",
    brief: "Website for a technology company that creates technology that bring the sensation of sound.",
    Description: "Project for a technology company that creates technology that bring the sensation of sound. This company is mainly focused on making premium quality headphones.\nThe website boasts the premium quality of their products with the help of complementing colors and fonts that just match the vibes and have suttle animation for making the user feel involved",
  },
  {
    id: 3,
    img: require("../images/autonoprj.png"),
    Heading: "Autono Project",
    brief: "Website for a technology company that creates technology that bring the sensation of sound.",
    Description: "Project for a technology company that creates technology that bring the sensation of sound. This company is mainly focused on making premium quality headphones.\nThe website boasts the premium quality of their products with the help of complementing colors and fonts that just match the vibes and have suttle animation for making the user feel involved",
  },
  {
    id: 4,
    img: require("../images/brwprj.png"),
    addClass: "RL",
    Heading: "BAR Project",
    brief: "Website for a technology company that creates technology that bring the sensation of sound.",
    Description: "Project for a technology company that creates technology that bring the sensation of sound. This company is mainly focused on making premium quality headphones.\nThe website boasts the premium quality of their products with the help of complementing colors and fonts that just match the vibes and have suttle animation for making the user feel involved",
  },
];
export default portfolioData;
