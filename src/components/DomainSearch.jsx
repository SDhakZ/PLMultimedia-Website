...
function DomainSearch() {
    ...
    const [informUser, setInformUser] = useState("Enter your domain to check if it is available");
    ...
    const getData = (callback) =>{
        ...
        fetch(`https://domain-checker7.p.rapidapi.com/whois?domain=${domain}`, options)
    ...
    const displayInfo = () => {
        ...
        if ( domainInfo.valid === false) {
            setInformUser("The domain you have entered is invalid! please try again.");
            setInfoColor("red");
        }
        else {
            if (domainInfo.available === false) {
                setInformUser("The domain you have entered is already taken! try another domain.")
                setInfoColor("red")
            }
            else if (domainInfo.available === true) {
                setInformUser("The domain you have entered is available!!! Contact us to register your domain.");
                setInfoColor("green");
            }
            else {
    ...
    return (
        ...
        <div className={DomainCSS.domainSearchSection}>
            ...
            <div className={DomainCSS.domHeader}>
                <img className={DomainCSS.domHeadImg} src={require("../logo/www.png")} alt="logo here"/>
                <h1 className={DomainCSS.domHeadTitle}>Search The Availability of Your Domain</h1>
            </div>

                <p className={DomainCSS.informUser} style={{color: infoColor}}>{informUser}</p>

            <div className={DomainCSS.domFooter}>
                <input className={DomainCSS.textField} onChange={(text)=> updateDomain(text.target.value)} placeholder="Enter domain name..."></input>
                <button className={DomainCSS.searchButton} onClick={()=> getData(displayInfo)}>Search</button>
            </div> 
        </div>
    );
